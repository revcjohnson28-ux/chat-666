...
function publicUser(u,online=false,socketId=null){return {username:u.username,role:u.role,avatar:u.avatar,bio:u.bio||'',online, socketId};}
...
function usersInRoom(room){return [...online.entries()].filter(([,v])=>v.room===room).map(([sid,v])=>publicUser(store.users[key(v.username)],true,sid));}
...
io.on('connection',socket=>{
  ...
  socket.on('join',data=>{
    ...
    online.set(socket.id,{username:u.username,room:roomName});socket.join(roomName);
    socket.emit('state',{me:publicUser(u,true,socket.id),room:store.rooms[roomName],rooms:roomInfo(),history:store.messages[roomName]||[]});
  ...
  socket.on('switch_room',room=>{const state=online.get(socket.id);if(!state||!store.rooms[room]||room===state.room)return;const old=state.room;socket.leave(old);state.room=room;socket.join(room);socket.emit('state',{me:publicUser(u,true,socket.id),room:store.rooms[room],rooms:roomInfo(),history:store.messages[room]||[]});socket.to(old).emit('system',`${u.username} left the room.`);socket.to(room).emit('system',`${u.username} entered the room.`);broadcastUsers(old);broadcastUsers(room);emitCounts();});
  ...
  socket.on('disconnect',()=>{const state=online.get(socket.id);if(!state)return;online.delete(socket.id);socket.to(state.room).emit('system',`${u.username} disconnected.`);broadcastUsers(state.room);emitCounts();});
});

server.listen(PORT,HOST,()=>console.log(`Eden's Viper Network v3 listening on http://${HOST}:${PORT}`));
